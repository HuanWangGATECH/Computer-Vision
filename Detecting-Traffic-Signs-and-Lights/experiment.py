...
def draw_tl_center(image_in, center, state):
    ...
    img_out = np.copy(image_in)
    image_cols, image_rows, _ = img_out.shape
    img_out = cv2.circle(img_out,center, 2, (0,0,0), -1)
    img_out = cv2.putText(img_out,'('+str(center)+','+"'"+state+"'"+ ')',
        (int(center[0]+50) + (int(center[0]+50)//image_cols) * (-300),center[1]),font,0.5,(0,0,0),1,cv2.LINE_AA)
    # cv2.imshow('img_temp', img_out)
    return img_out
...
def mark_traffic_signs(image_in, signs_dict):
    ...
    for key in signs_dict:
        img_out = cv2.circle(img_out,signs_dict[key], 2, (0,0,0), -1)
        img_out = cv2.putText(img_out,'('+str(signs_dict[key])+','+"'"+key+"'"+ ')',
                                    (int(signs_dict[key][0])-30 + ((int(signs_dict[key][0])-30)//image_cols) * (-50),int(signs_dict[key][1] + 40)),
