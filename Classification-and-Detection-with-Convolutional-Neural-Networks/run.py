...
def non_max_suppression(box_list, threshold = 0.01):
	remain_box = []
...
def sliding_window_1(image, stepSize, windowSize, ratio):
...
def sliding_window_2(image, stepSize, windowSize, y_1, x_1, y_2, x_2, ratio, x_ini):
...
CNNmodel_1 = load_model('pretrained/VGGPreTrained_2_round.hdf5')
CNNmodel_2 = load_model('pretrained/VGG.classifier.hdf5')
...
for i in range(5):
	if i == 4:#i <= 4:
		...
		if i == 2 or i == 4:
			...
			steptemp = 10
		for (x, y, window) in sliding_window_1(image, stepSize=steptemp, windowSize=(100, 100), ratio = 1.0):
			clone = image.copy()
			img_parts = np.reshape(window/255, (1,100,100,3))
			yOut = CNNmodel_1.predict(img_parts)
			...
			if yOut[0][0] > 0.99:
				...
				fontColor = (0, 255, 0)
				lineType  = 2
				cv2.putText(image, str(round(yOut[0][0],2)), center, font, fontScale, fontColor, lineType)
		...
		if len(box_list) == 0:
			...
			for (x, y, window) in sliding_window_1(image_2, stepSize=5, windowSize=(100, 100), ratio = 0.7):
				clone = image_2.copy()
				img_parts = np.reshape(window/255, (1,100,100,3))
				yOut = CNNmodel_1.predict(img_parts)
				cv2.rectangle(clone, (x, y), (x + 100, y + 100), (0, 255, 0), 1)
				...
				if yOut[0][0] > 0.99:
					...
					fontColor = (0, 255, 0)
					lineType  = 2
					cv2.putText(image_2, str(round(yOut[0][0],2)), center, font, fontScale, fontColor, lineType)
					x_1 = int(x/0.7)
					y_1 = int(y/0.7)
					x_2 = int((x+100)/0.7)
					y_2 = int((y+100)/0.7)
		...
		if len(box_list) == 0:
			print('3')	
			image_3 = cv2.resize(image_2, (0,0), fx=0.7, fy=0.7) 
			for (x, y, window) in sliding_window_1(image_3, stepSize=10, windowSize=(100, 100), ratio = 0.49):
				clone = image_3.copy()
				img_parts = np.reshape(window/255, (1,100,100,3))
				yOut = CNNmodel_1.predict(img_parts)
				cv2.rectangle(clone, (x, y), (x + 100, y + 100), (0, 255, 0), 1)
				...
				if yOut[0][0] > 0.99:
					cv2.rectangle(image_3, (x, y), (x + 100, y + 100), (0, 255, 0), 1)
					font = cv2.LINE_AA
					center = (x, y)
					fontScale = 0.5
					fontColor = (0, 255, 0)
					lineType  = 2
					cv2.putText(image_3, str(round(yOut[0][0],2)), center, font, fontScale, fontColor, lineType)
					x_1 = int(x/0.49)
					y_1 = int(y/0.49)
					x_2 = int((x+100)/0.49)
					y_2 = int((y+100)/0.49)
					cv2.rectangle(image, (x_1, y_1), (x_2, y_2), (0, 255, 0), 1)
					box_info = [yOut[0][0], y_1, x_1, y_2, x_2, 0.49]
					box_list.append(box_info)
		...
		remain_box = non_max_suppression(box_list)
		count = 0
		for box in remain_box:
			...
			y_1 = int(y_1*ratio_scale)
			x_1 = int(x_1*ratio_scale)
			y_2 = int(y_2*ratio_scale)
			x_2 = int(x_2*ratio_scale)
			...
			if i >= 3:
				...
				ss = 8
			for (x, y, window) in sliding_window_2(output, ss, 32, y_1, x_1, y_2, x_2, box[5],x_ini):
				output_2 = np.copy(output)
				output_3 = np.copy(output_3)
				img_parts = np.reshape(window/255, (1,32,32,3))
				yOut = CNNmodel_2.predict(img_parts)
				pmax = np.max(yOut)
				...
				cv2.rectangle(output_2, (x, y), (x + 32, y + 32), (0, 255, 0), 1)
				if pmax > 0.952: #change
					cv2.putText(output_2, string, testLoc, font, fontScale, fontColor, lineType)
			...
			for key,value in Map.items():
				...
				remain_box = non_max_suppression(value, threshold = 0.8)
			...
			for key, remain_box in new_map:
				for box in remain_box:
					...
					if (box[1]==1 and np.abs(box[3] - temp) > 5) or box[1] != 1:
						...
						cv2.rectangle(output_final, testLoc, (x + win_size, y + win_size), (0, 255, 0), 1)
						cv2.putText(output_final, str(pmax_index), testLoc, font, fontScale, fontColor, lineType)
						# cv2.putText(output_final, str(round(pmax, 2)), PLoc, font, 0.4, fontColor, 1)
			
			# cv2.rectangle(output_final, (x_min - 20, y_min - 20), (x_max + 20 , y_max+ 20), (255, 0, 0), 2)
			# cv2.putText(output_final, res, (x_min - 20, y_min - 30), font, 1, (255, 0, 0), lineType)
			cv2.imwrite('InputImages/test'+ str(i+1) + '.png', output_final)
